...
describe('getBodyData', () => {
  ...
  it('should extract text/plain from parts[0]', () => {
    ...
    const message = makeMessage({
      ...
      parts: [
        { mimeType: 'text/plain', body: { data: encode(text), size: text.length } },
        { mimeType: 'text/html', body: { data: encode('<p>Hello</p>'), size: 12 } },
      ],
    });
  ...
  it('should extract text/plain from parts[1] when parts[0] is not text', () => {
    ...
    const message = makeMessage({
      mimeType: 'multipart/mixed',
      parts: [
        { mimeType: 'image/png', body: { data: encode('png'), size: 3 }, filename: 'a.png' },
        { mimeType: 'text/plain', body: { data: encode(text), size: text.length } },
      ],
    });
          ...
          parts: [
            {
              ...
              parts: [
                { mimeType: 'text/plain', body: { data: encode(text), size: text.length } },
                { mimeType: 'text/html', body: { data: encode('<p>Yandex</p>'), size: 13 } },
              ],
            },
          ],
        },
  ...
  it('should skip text/plain attachments (has filename + attachmentId)', () => {
    ...
    const message = makeMessage({
      ...
      parts: [
        ...
        { mimeType: 'text/plain', body: { data: encode(bodyText), size: bodyText.length } },
      ],
    });
  ...
  it('should return last text/plain when multiple exist', () => {
    ...
    const message = makeMessage({
      mimeType: 'multipart/mixed',
      parts: [
        { mimeType: 'text/plain', body: { data: encode(first), size: first.length } },
        { mimeType: 'text/plain', body: { data: encode(second), size: second.length } },
      ],
    });
